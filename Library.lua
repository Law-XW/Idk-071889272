...
local Library = {
    ...
    IsLightTheme = false,
    Scheme = {
        BackgroundColor = Color3.fromRGB(5, 0, 8),
        MainColor = Color3.fromRGB(15, 5, 18),
        AccentColor = Color3.fromRGB(255, 20, 147),
        OutlineColor = Color3.fromRGB(80, 10, 50),
...
function Library:CreateWindow(WindowInfo)
    ...
    if Library.IsMobile then
        ...
        do
            local Btn = ToggleButton.Button
            -- Square button: remove corner from corners list and set radius to 0
            for i = #Library.Corners, 1, -1 do
                if Library.Corners[i].Parent == Btn then
                    Library.Corners[i].CornerRadius = UDim.new(0, 0)
                    break
                end
            end
            -- Replace text with an icon (menu icon using Unicode)
            Btn.Text = "☰"
            Btn.TextSize = 22
